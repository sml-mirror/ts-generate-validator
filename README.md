...
`unknownPropertySeverityLevel`_(number)_ - уровень проверки неизвестных параметров (параметров, имеющих 1) тип в виде ссылки на другие классы моделей без `@Validation` декораторов или 2) не поддерживаемые типы без `@CustomValidation` декораторов). Может иметь следующие значения:
...
```json
{
  ...
  "unknownPropertySeverityLevel": "error"
}
```
